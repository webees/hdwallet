...
func NewUnsignedTransaction(outputs []*wire.TxOut, feeRatePerKb btcutil.Amount,
	...
	for {
		...
		if changeAmount != 0 && !txrules.IsDustAmount(changeAmount,
			txsizes.RedeemNestedP2WPKHScriptSize, txrules.DefaultRelayFeePerKb) {
			changeScript, err := fetchChange()
			if err != nil {
				return nil, err
			}
			if len(changeScript) > txsizes.RedeemNestedP2WPKHScriptSize { // https://github.com/btcsuite/btcwallet/blob/master/wallet/txauthor/author.go#L141
