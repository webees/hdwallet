...
// 未签名raw
func UnsignedRaw(nonce uint64, to string, amount string, gasLimit uint64, gasPrice uint64, fromIndex int, data []byte) (string, error) {
	bGasPrice := big.NewInt(int64(gasPrice))
	toAddr := common.HexToAddress(to) // 发送地址
...
// ERC20转账
func ERC20unsignedRaw(nonce uint64, tokenAddress string, toAddress string, erc20amount string, gasLimit uint64, gasPrice uint64, frIndex int) (string, error) {
	data := erc20data(toAddress, erc20amount)
	s, e := UnsignedRaw(nonce, tokenAddress, strconv.Itoa(0), gasLimit, gasPrice, frIndex, data)
	if e != nil {
		return "", e
	}
	return s, nil
